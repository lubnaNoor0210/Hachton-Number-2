...
interface BlogCardProps {
  imageSrc: string; // Define imageSrc as a prop
}

const BlogCard: React.FC<BlogCardProps> = ({ imageSrc }) => {
  return <img src={imageSrc} alt="Blog Post" />;
}; 
...
const HeroPage = () => {
  ...
  const recentPosts = [
    { image: "/Blogpage2.1.png", date: "June 22, 2020" },
    { image: "/Blogpage2.1.png", date: "June 23, 2020" },
  ...
  {[FaFacebook, FaTwitter, FaInstagram].map((Icon: React.ComponentType<React.SVGProps<SVGSVGElement>>, index) => (
    <Button
      key={index}
      ...
      <Icon className="text-white text-lg md:text-xl" />
            ...
            <div className="bg-white border border-gray-300 p-4 md:p-6 rounded-lg text-center">
              ...
              <div className="flex justify-center gap-4">
                ...
                ].map(({Icon,bg} : {Icon: React.ComponentType<React.SVGProps<SVGSVGElement>>, bg: string}, index) => (
                  <Button key={index} className="p-2 md:p-3 bg-gray-700 rounded-full hover:bg-[#FF9F0D] transition-colors duration-300">
    <Icon className="text-white text-lg md:text-xl" />
  </Button>
                ))}
              </div>
